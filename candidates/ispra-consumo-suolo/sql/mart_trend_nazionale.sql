-- mart_trend_nazionale.sql — ispra_consumo_suolo
-- Trend annuale nazionale del consumo di suolo (solo periodi annuali 2016-2024)

SELECT
    anno,
    COUNT(*)                                            AS comuni,
    ROUND(AVG(stock_pct), 3)                            AS avg_stock_pct,
    ROUND(AVG(stock_ha), 1)                             AS avg_stock_ha,
    ...
    ROUND(AVG(incremento_lordo_ha), 2)                  AS avg_inc_lordo_ha,
    ROUND(SUM(incremento_lordo_ha), 1)                  AS tot_inc_lordo_ha,
    ROUND(AVG(ripristino_ha), 2)                        AS avg_ripristino_ha,
