...
| `pr-toolkit-check.yml` | PR su `candidates/` | `toolkit run all` + `validate all` + gate blocker_hints |
| `post-merge-candidate.yml` | Merge su `candidates/` | Run CI, rebuild `pipeline_signals.json`, apre draft PR handoff |
...
| `sample-candidate-run.yml` | Workflow dispatch | Run di esempio su candidate specifico |
